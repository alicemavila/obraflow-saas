...
import { ChevronLeft, Plus, MapPin, User, Calendar, ClipboardList } from 'lucide-react'
...
export default async function ObraDetailPage({ params }: { params: { id: string } }) {
  ...
  const ROLE_LABELS: Record<string, string> = {
    GESTOR_OBRA: 'Gestor', COLABORADOR: 'Colaborador', CLIENTE_SINDICO: 'Cliente/Síndico',
    ADMIN_EMPRESA: 'Admin', SUPER_ADMIN: 'Super Admin',
  }
  const LOG_STATUS_LABELS: Record<string, string> = {
    RASCUNHO: 'Rascunho', SUBMETIDO: 'Aguardando', APROVADO: 'Aprovado', REJEITADO: 'Rejeitado',
  }
  const LOG_STATUS_VARIANTS: Record<string, string> = {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center gap-2 text-sm text-gray-500">
        <Link href="/obras" className="flex items-center gap-1 hover:text-blue-700"><ChevronLeft className="w-4 h-4" aria-hidden />Obras</Link>
        <span>/</span>
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          {worksite.clientName && <p className="text-gray-500 text-sm mt-1">Cliente: {worksite.clientName}</p>}
        </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          <Card>
            ...
            <CardContent className="space-y-2 text-sm">
              {[
                { icon: MapPin, label: 'Endereço', value: `${worksite.address}, ${worksite.neighborhood ? worksite.neighborhood + ', ' : ''}${worksite.city} - ${worksite.state}` },
                { icon: User, label: 'Responsável', value: `${worksite.responsibleName}${worksite.responsibleCrea ? ` (${worksite.responsibleCrea})` : ''}` },
                { icon: Calendar, label: 'Início', value: formatDate(worksite.startDate) },
                ...(worksite.endDateForecast ? [{ icon: Calendar, label: 'Previsão', value: formatDate(worksite.endDateForecast) }] : []),
                ...(worksite.artNumber ? [{ icon: ClipboardList, label: 'ART/RRT', value: worksite.artNumber }] : []),
                ...(worksite.contractNumber ? [{ icon: ClipboardList, label: 'Contrato', value: worksite.contractNumber }] : []),
              ].map(row => (
                <div key={row.label} className="flex items-start gap-2">
                  <row.icon className="w-4 h-4 text-gray-400 mt-0.5 flex-shrink-0" aria-hidden />
          ...
          <Card>
            <CardHeader className="pb-3 flex flex-row items-center justify-between">
              ...
              <Link href={`/obras/${worksite.id}/diarios`} className="text-xs text-blue-600 hover:underline">Ver todos ({worksite._count.dailyLogs})</Link>
            </CardHeader>
            <CardContent>
              {recentLogs.length === 0 ? (
                ...
                <div className="space-y-2">
                  {recentLogs.map(log => (
                    <Link key={log.id} href={`/diarios/${log.id}`} className="flex items-center justify-between p-2 rounded-lg hover:bg-gray-50 group">
                      ...
                      <Badge variant={LOG_STATUS_VARIANTS[log.status] as never}>{LOG_STATUS_LABELS[log.status]}</Badge>
                    </Link>
                  ))}
                </div>
              )}
